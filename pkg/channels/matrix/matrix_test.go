...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("qclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@qclaw hello", want: true},
		{text: "hi @qclaw:matrix.org", want: true},
		{
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@qclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@qclaw:matrix.org hello", want: "hello"},
		{in: "@qclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@qclaw:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@qclaw:matrix.org">QClaw</a> hello`,
			},
			want: true,
		},
		{
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@qclaw:matrix.test"), "")
	if err != nil {
