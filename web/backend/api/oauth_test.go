...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("QCLAW_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("QCLAW_HOME", filepath.Join(tmp, ".qclaw")); err != nil {
		t.Fatalf("set QCLAW_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("QCLAW_HOME")
		} else {
			_ = os.Setenv("QCLAW_HOME", oldPicoHome)
		}
	}
