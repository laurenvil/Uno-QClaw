	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					qclawconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
...
func (s *appState) addModel(model qclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []qclawconfig.ModelConfig) {
...
func isModelValid(model qclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *qclawconfig.ModelConfig) {
	if model == nil {
		return
	}
