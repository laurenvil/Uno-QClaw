#!/usr/bin/env bash
#
# bench-run12-ud-q4kxl.sh — Run 12: full 9-prompt agentic battery,
# Qwen3.5-0.8B-UD-Q4_K_XL.gguf (559 MB, unsloth dynamic Q4_K_XL)
#
# Flags:
#   --threads 4  --ctx-size 9000  --temperature 0.3
#   --top-p 0.9  --top-k 20  --min-p 0.0
#   --repeat-penalty 1.0
#   --reasoning off  --reasoning-budget 0
#   /no_think in SOUL.md (retained from run 11)
#
# Temporarily patches ~/.qclaw/config.json to add the yzma-q4kxl model
# entry and set it as default; restores on exit.
#
# Usage: cd <repo-root> && bash scripts/bench-run12-ud-q4kxl.sh

set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

BINARY="./build/qclaw"
LOG_DIR="docs/benchmarks/run12/raw"
TSV="docs/benchmarks/run12/timing.tsv"
SERVER_PORT=8084
METRICS_URL="http://127.0.0.1:${SERVER_PORT}/metrics"
CONFIG="${HOME}/.qclaw/config.json"
CONFIG_BAK="${HOME}/.qclaw/config.json.run12.bak"
...
print("  Config patched: yzma-q4kxl added, set as default.")
PYEOF
...
log "Run 12 — UD-Q4_K_XL 9-prompt agentic battery · ctx 12000"
log "Binary : $BINARY"
log "Logs   : $LOG_DIR"
log "Model  : yzma-q4kxl (Qwen3.5-0.8B-UD-Q4_K_XL.gguf, 559 MB, ctx 9000, port $SERVER_PORT)"
log "Flags  : --reasoning off --reasoning-budget 0 --temp 0.3 --top-p 0.9 --top-k 20 --min-p 0.0 --repeat-penalty 1.0"
log ""
...
rm -f ~/.qclaw/workspace/sessions/run12-p* 2>/dev/null || true
...
for i in "${!TAGS[@]}"; do
    ...
    BEFORE_M=""
    curl -sf --max-time 2 "$METRICS_URL" > /tmp/r12_before.txt 2>/dev/null \
        && BEFORE_M=$(cat /tmp/r12_before.txt) || true
    ...
    AFTER_M=""
    curl -sf --max-time 2 "$METRICS_URL" > /tmp/r12_after.txt 2>/dev/null \
        && AFTER_M=$(cat /tmp/r12_after.txt) || true
    ...
    elif printf '%s\n' "$CLEAN" | grep -q "request.*exceeds.*context"; then
        STATUS="ctx_overflow"
    else
        STATUS="ok"
    fi
