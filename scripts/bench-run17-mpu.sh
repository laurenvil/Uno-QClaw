#!/usr/bin/env bash
#
# bench-run17-mpu.sh — Run 17: full 9-prompt agentic battery,
# engines/llamacli/mpu/llama-server (e6ed0a2, GPU-CLI Q4_0 noshuffle kernels,
# static CPU fallback) with Q4_0 model and run-15/16 flags.
#
# Config: llamacli-mpu · ctx 9000 · port 8086 · --flash-attn on · --mlock
#         --cache-type-k q8_0 · --cache-type-v q8_0 · --reasoning-budget 800
#         --repeat-penalty 1.1 · --repeat-last-n 64
#
# Usage: cd <repo-root> && bash scripts/bench-run17-mpu.sh

set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

BINARY="./build/qclaw"
LOG_DIR="docs/QClaw/v2/benchmarks/run17/raw"
TSV="docs/QClaw/v2/benchmarks/run17/timing.tsv"
SERVER_PORT=8086
...
log "Run 17 — llamacli-mpu (e6ed0a2, GPU-CLI noshuffle, static CPU) · Q4_0 · ctx 9000 · full 9-prompt agentic battery"
log "Binary : $BINARY"
log "Logs   : $LOG_DIR"
log "Engine : engines/llamacli/mpu/llama-server"
log "Model  : Qwen_Qwen3.5-0.8B-Q4_0.gguf (490 MB, ctx 9000, port $SERVER_PORT)"
...
rm -f ~/.qclaw/workspace/sessions/run17-p* 2>/dev/null || true
